...
/**
 * EventController 整合測試 (使用 H2 內存資料庫)
 * 此測試不需要 Docker，適合在無 Docker 環境中執行
 */
@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles("test")
class EventControllerH2Test {
    ...
    fun `should filter events by age`() {
        mockMvc.perform(
            ...
            .andExpect(jsonPath("$.data[2].name").value("2026 兒童足球夏令營"))
            .andExpect(jsonPath("$.data[3].name").value("兒童羽球初階班"))
    }
